...
Cypress.Commands.add("deleteintegrationAPI", () => {
  cy.request({
    method: "GET",
    url: `/web/factoring/api/v2/integrations/factoringDetails?organizationId=${Cypress.env(
      "originitionID"
    )}`,
    ...
    //@ts-ignore
    const allIds = Object.values(cred).flatMap((array) =>
    //@ts-ignore
    ...
    allIds.forEach((id) => {
      cy.request({
        method: "DELETE",
        url: `/web/factoring/api/v2/integrations?id=${id}`,
        headers: { "Mp-Auth-Token": Cypress.env("accessToken") },
        failOnStatusCode: false,
      }).then((res1) => {
        console.log("response of deletion", id, res1.status);
      });
    });
  });
});
...
Cypress.Commands.add("createCustomerAPI", () => {
  for (let i = 0; i <= 0; i++) {
    // run if you want to create multiple customers
    customercreateAPI();
  }
});
...
Cypress.Commands.add(
  "loginx",
  (dot: string, email: string, password: string) => {
    cy.session("loginsession", () => {
