/// <reference types="cypress" />
  ...
  export function customerDeleteAPI(){
    ...
    cy.request({
        method: 'PUT',
        url: `web/customer/api/v2/customers/get/customer/list?organizationId=${Cypress.env('originitionID')}&pageNumber=1&pageSize=100&sort=%2Bname`,
        body: {
        ...
        id.forEach((num)=>{
            cy.request({
                method: 'DELETE',
                url: `/web/customer/api/v2/customers/customer?organizationId=${Cypress.env('originitionID')}&id=${num}`,
