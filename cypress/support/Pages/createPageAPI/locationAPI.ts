...
export function generateLocation(){
    cy.request({
        method: 'GET',
        url: `https://singlesearch.alk.com/NA/api/search?authToken=C262376E2A14FD4B88180F8B75A11E46&countryType=ISO&getAllHNRanges=false&maxResults=25&query=${fake('location')}&include=Meta&countries=US,%20CA,MX&useCustomPlaces=false&excludeResultsFor=RouteAlpha,POI,POIStreet,POIType,CustomPlace,CrossStreet,LatLon`,
       ...
       console.log('location: ', respone.body.Locations)
        ...
        console.log('firstLocation: ', locData[0], 'fullAddress: ', locData[0].ShortString, 'firstLocationAddress: ', locData[0].Address);
        console.log('lattitudeLogitude: ', locData[0].StreetCoords.Lat, ' lon: ', locData[0].StreetCoords.Lon, ' zip: ', locData[0].Address.Zip)
  }
        const pickUpbody = {
            "contacts": [{
                "fullName": fake('name'),
        ...
        cy.request({
            method: 'POST',
            url: `/web/location/api/v2/locations/create/location?organizationId=${Cypress.env('originitionID')}`,
            body: pickUpbody,
